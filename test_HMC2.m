...
% TODO: go line by line and check code.
% TODO: improve code (put sigmas explicitally, GSs, etc)
...
t_obs = linspace(0, 24 * 3600, 1000); % 1 hour, 100 samples
...
% Nominal state (could be MAP estimate or true_state)
x_nom0 = startpoint;
Phi0_flat = reshape(eye(6), [], 1);
x_aug0 = [x_nom0; Phi0_flat];

% Nominal propagation
[~, y_nom] = ode113(@(t, x) two_body(t, x, mu_earth), t_obs, x_aug0);

% Extract nominal state and STM
r_nom = y_nom(:, 1:3)';
v_nom = y_nom(:, 4:6)';
Phi_all = reshape(y_nom(:, 7:end)', 6, 6, []);

% Define log-posterior
logpdf = @(dx) logPosteriorOrbit(dx, t_obs, range_meas, ...
    rangerate_meas, r_canberra, mu_earth, r_nom, v_nom, Phi_all);
startpoint = zeros(6,1); % zero deviation
smp = hmcSampler(logpdf, startpoint, 'NumSteps', 10, 'StepSize', 0.01, 'CheckGradient', true);

%% MAP estimation
[MAPpars, fitInfo] = estimateMAP(smp, 'Start', startpoint, 'VerbosityLevel', 2);
...
function [logpdf, gradlogpdf] = logPosteriorOrbit(dev_x0, t_obs, ...
    rho_meas, drho_meas, r_station, ~, r_nom, v_nom, Phi_all)
    ...
    for i = 1:N
        Phi = Phi_all(:,:,i);
        delta_x = Phi * dev_x0;
        r = r_nom(:,i) + delta_x(1:3);
        v = v_nom(:,i) + delta_x(4:6);
    ...
    prior_std = [1; 1; 1; 0.1; 0.1; 0.1]; % km and km/s
    prior_inv = diag(1 ./ prior_std.^2);
    logprior = -0.5 * (dev_x0' * prior_inv * dev_x0);
    dlogprior = -prior_inv * dev_x0;
