"""
test_Claude.py  —  Non-Gaussian SPH posterior scenario (generated by Claude Sonnet 4.6)

PURPOSE
-------
A deliberate scenario designed so that the spherical-harmonics posterior is
provably non-Gaussian.  Everything else is identical to
J1_scenarioA3_radio_SPH3_Bennu_popper.py (same dynamics, same SPICE kernels,
same pipeline); only the observation settings, noise level, priors, and detachment
geometry are changed.

PHYSICAL REASONING FOR NON-GAUSSIANITY
---------------------------------------
The posterior is Gaussian when (i) the measurement model is linear in the
parameters and (ii) the posterior is narrow enough to stay in the linear regime.
Even with STT order=1 the range/range-rate function H is a *nonlinear* function
of position (H_range = |r_particle − r_SC|).  When the posterior is broad, its
walkers explore regions where H curvature is non-negligible → non-Gaussian shapes.
The following choices force the posterior to be broad and to sit in the nonlinear
regime of H:

1. SHORT ARC (45 min << Bennu spin period 4.3 hr)
   Degree-3 SH terms (C33, S33) modulate at 3× the spin rate (period ≈ 86 min).
   With only 45 min of data we see <0.6 cycles → SNR on C33/S33 < 1 at realistic
   noise → their posteriors spread to the full prior width.

   Estimated signal: F_C33 ≈ 1.2e-10 km/s², integrated over 45 min gives
   Δρ ≈ 0.44 m.  With σ_range = 1e-3 km = 1 m this is SNR < 1.

2. REALISTIC DSN-CLASS NOISE (σ_range=1e-3 km, σ_range_rate=1e-8 km/s)
   ~100–1000× worse than the futuristic original.  Degree-3 coefficients become
   marginally detectable at best; degree-2 partially constrained.  Broad posteriors
   expose the curvature of H.

3. NEAR-EQUATORIAL DETACHMENT AT BODY-FIXED LONGITUDE λ₀ = 45°
   At λ₀ = 45°: cos(2λ₀) = 0, so C22 contributes *zero force* exactly at t=0.
   As Bennu rotates under the particle (ω × 45 min ≈ 63°), the body-fixed longitude
   sweeps from 45° to −18°, and C22 sensitivity grows nonlinearly throughout the arc.
   This creates a curved (non-ellipsoidal) joint posterior in the (C22, S22) plane.

   Simultaneously, near-equatorial latitude means P₃₀(sin 0°) ≈ 0 and
   P₃₁(sin 0°) ≈ 0 → C30 and C31 are nearly unobservable from this trajectory;
   their posteriors are essentially flat (prior-limited).

4. VERY LOOSE SH PRIORS (10× truth value)
   Removes the Gaussian straitjacket that normally keeps the posterior narrow.
   The MCMC explores the full nonlinear regime of the likelihood.

5. CLASSICAL (µ, C20) DEGENERACY — "BANANA" POSTERIOR
   For short arcs, µ and C20 both affect the mean radial acceleration (they appear
   as µ/r² and µ/r² × (R/r)² × C20 × (3sin²φ − 1)/2).  At near-equatorial
   latitudes the C20 term reduces to −µ/r² × (R/r)² × C20/2, which mimics a
   renormalization of µ.  The likelihood contours in the (µ, C20) plane are curved
   (banana-shaped), a classic short-arc OD result.

6. SMALL RADIAL VELOCITY FRACTION (rad_frac = 0.03)
   With a tiny radial kick and only 45 min of data, the data cannot cleanly
   distinguish positive vs negative radial components → adds mild bimodal tendency
   to the velocity posteriors that couples into the SH estimates.

WHAT TO LOOK FOR IN THE CORNER PLOT
-------------------------------------
- (µ, C20): banana-shaped 2D marginal (curved degeneracy)
- (C22, S22): elongated, rotated, possibly non-ellipsoidal contour
- C30, C31, S31: nearly flat (prior-dominated), possibly skewed tails
- C33, S33: essentially flat / Gaussian but very broad (SNR < 1)
- Off-diagonal correlations between degree-2 and degree-3 terms

Same pipeline as the original:
  1) Propagate truth GravityPopper (19D) via STTPropagatorND.
  2) SPICE query for SC truth state.
  3) Noisy radiometric measurements + occultation mask.
  4) Reference x0_ref ≈ truth.
  5) Stage-1 Gauss-Newton batch MAP (STM-based).
  6) Stage-2 relinearise about Stage-1 MAP (x0_ref1).
  7) MCMC (emcee) sampling the nonlinear posterior.
...
# ============================================================
# Gaussian-mixture likelihood helpers (no subclassing needed —
# just patch model.log_likelihood after MCMCModel construction)
# ============================================================
...
def make_mixture_log_likelihood(residuals_func, epsilon, k_outlier):
    """
    Return a picklable log-likelihood callable for the contamination model

        p(rᵢ) = (1−ε)·N(rᵢ;0,1) + ε·N(rᵢ;0,k²)
...
if __name__ == "__main__":
    ...
    # --------------------------
    # USER SETTINGS — REALISTIC PROXIMITY-OPS SCENARIO
    # Demonstrates what MCMC buys you over a Kalman filter:
    #   1) Non-Gaussian (Uniform) priors on the SH coefficients
    #   2) Robust Student-t likelihood — automatic outlier downweighting
    # --------------------------
    ...
    # (detector glitches, stray light, mis-identification of target).
    # A Kalman filter treats all measurements equally; MCMC + mixture likelihoods
    # automatically down-weights large outliers without manual data editing.
    ...
    # mu: UNIFORM, ±30% of true value — bounded ignorance analogous to SH coefficients
    mu_prior_half_width = np.abs(mu_true) * 0.3
    # SH coefficients: UNIFORM bounded at ±3×|truth| for degree-2,
    #                  ±5×|truth| for degree-3.
    # Rationale: a Kalman filter must use Gaussian priors; MCMC can represent
    # "bounded ignorance" (we know the order of magnitude but not the sign).
    sh_scale = np.where(
        np.arange(12) < 5,  # indices 0-4: C20,C21,S21,C22,S22 (all deg-2)
        3.0,
        5.0,  # 3× for deg-2, 5× for deg-3
    )
    sh_prior_half_width = sh_scale * np.abs(params_true[1:])
    ...
    sh_prior_half_width = np.maximum(sh_prior_half_width, sh_floor)
    ...
    # At lon=45: cos(2*45)=0 so C22 contributes zero force at t=0.
    # As Bennu rotates under the particle (~63 deg in 45 min), C22 sensitivity
    # builds nonlinearly -> curved (C22, S22) joint posterior.
    # Near-equatorial latitude means P30(sin~0)~0, P31(sin~0)~0 -> C30 and C31
    # nearly unobservable from this trajectory -> flat, prior-dominated marginals.
    ...
    # This mimics detector glitches or stray-light blinding of the camera.
    # The Student-t likelihood will automatically downweight these; a Kalman
    # filter (Gaussian assumption) would bias its solution toward the outliers.
    ...
    # --------------------------
    # --- Build priors on 19D delta-state (deviation about x0_ref) ---
    # Position + velocity + mu: Gaussian (physically motivated error model)
    priors_r = [norm(loc=0.0, scale=s) for s in sig_prior_r]
    priors_v = [norm(loc=0.0, scale=s) for s in sig_prior_v]
    priors_mu = [scipy_uniform(loc=-mu_prior_half_width, scale=2.0 * mu_prior_half_width)]

    # SH coefficients: UNIFORM on [-hw, +hw]   (bounded ignorance)
    # scipy.stats.uniform(loc=a, scale=b-a) gives support [a, a+scale]
    # => loc=-hw, scale=2*hw gives support [-hw, +hw]
    priors_sh = [scipy_uniform(loc=-hw, scale=2.0 * hw) for hw in sh_prior_half_width]
    ...
    print("[Prior] mu half-width (Uniform) [km³/s²]:", mu_prior_half_width)
    print("[Prior] SH half-widths (Uniform):", sh_prior_half_width)
    ...
    # Priors for MCMC — shifted to be centered on the Stage-1 MAP delta=0
    # For Gaussian priors the center shifts by delta_shift; for Uniform priors
    # the center is already delta=0 (we allow the same ±hw from the MAP estimate).
    delta_shift = x0_ref1 - x0_ref
    priors_ref1 = []
    for i, prior in enumerate(priors):
        if i < 6:
            # Gaussian (r, v): shift center to account for Stage-1 offset
            priors_ref1.append(norm(loc=-delta_shift[i], scale=prior.std()))
        elif i == 6:
            # mu: Uniform — keep symmetric about delta=0 (MAP is our new reference)
            priors_ref1.append(scipy_uniform(loc=-mu_prior_half_width, scale=2.0 * mu_prior_half_width))
        else:
            # SH coefficients: Uniform — keep symmetric about delta=0
            hw = sh_prior_half_width[i - 7]
            priors_ref1.append(scipy_uniform(loc=-hw, scale=2.0 * hw))

    # --------------------------
    # MCMC with Student-t likelihood (ν=nu_student_t)
    # Replacing MCMCModel with RobustMCMCModel is the only change needed
    # to switch from Gaussian to Student-t — the MCMC infrastructure is identical.
    # --------------------------
    print(
        f"\n[MCMC] Running with Gaussian-mixture likelihood "
        f"(ε={outlier_frac:.2f}, k={outlier_scale:.0f}σ) and Uniform SH priors..."
    )
    model = MCMCModel(
    ...
    # Instance attributes shadow class methods, so self.log_likelihood(theta)
    # inside MCMCModel will call this function instead of the Gaussian default.
    model.log_likelihood = make_mixture_log_likelihood(
    ...
    # Per-epoch outlier probabilities from the mixture model
    p_out = mixture_outlier_probs(
        residuals_normalized, outlier_frac, outlier_scale, theta_hat
    )
    p_out_ra = p_out[0::2]  # one value per epoch (RA residual drives it)
    t_hr = tau / 3600.0
    ...
    ax.axhline(
        outlier_frac, color="k", ls="--", lw=1.2, label=f"Prior $\\varepsilon$={outlier_frac:.2f}"
    )
